...
const input = { width: "100%", border: "1px solid #F1EBDD", borderRadius: 10, padding: "11px 13px", background: "#FFFFFF", color: "#1A1A1A" } as const;
const card = { background: "#FFFFFF", border: "1px solid #F1EBDD", borderRadius: 18, padding: 24, boxShadow: "0 8px 30px rgba(26,26,26,.05)" } as const;
const button = { border: 0, borderRadius: 999, padding: "10px 16px", cursor: "pointer", fontWeight: 700 } as const;
const muted = { color: "#5C3C2B", fontSize: 14, lineHeight: 1.6 } as const;
...
function Page({ children }: { children: React.ReactNode }) { return <main style={{ minHeight: "100vh", background: "#F1EBDD", color: "#1A1A1A", fontFamily: "Arial, sans-serif", padding: "32px 18px" }}><div style={{ maxWidth: 1180, margin: "0 auto" }}>{children}</div></main>; }
...
function Login({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  return <Page><div style={{ maxWidth: 430, margin: "8vh auto" }}><div style={{ textAlign: "center", marginBottom: 26 }}><div style={{ color: "#C98A3D", fontSize: 12, letterSpacing: 3 }}>ӨГӨӨМЖ АРХАД ХХК</div><h1 style={{ fontFamily: "Georgia, serif", fontSize: 38, margin: "12px 0 8px" }}>Admin нэвтрэх</h1><p style={muted}>Сайтын контент, бүтээгдэхүүн, зургийг удирдана.</p></div><form onSubmit={submit} style={{ ...card, display: "grid", gap: 14 }}><label>Имэйл<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} style={input} /></label><label>Нууц үг<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoFocus style={input} /></label>{error && <div style={{ color: "#F00028", background: "#F1EBDD", padding: 11, borderRadius: 9 }}>{error}</div>}<button style={{ ...button, background: "#F00028", color: "white" }}>Нэвтрэх</button></form></div></Page>;
}

function Dashboard({ onLogout }: { onLogout: () => void }) {
  const [tab, setTab] = useState("content");
  return <Page><header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap", marginBottom: 22 }}><div><div style={{ color: "#C98A3D", fontSize: 12, letterSpacing: 2 }}>ӨГӨӨМЖ АРХАД ХХК</div><h1 style={{ fontFamily: "Georgia, serif", fontSize: 36, margin: "8px 0 0" }}>Сайтын удирдлага</h1></div><div style={{ display: "flex", gap: 8 }}><a href="/" style={{ ...button, background: "#F1EBDD", color: "#F00028", textDecoration: "none" }}>Сайт харах</a><button onClick={onLogout} style={{ ...button, background: "#F00028", color: "white" }}>Гарах</button></div></header><nav style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>{[["content", "Текст засах"], ["products", "Бүтээгдэхүүн"], ["settings", "Тохиргоо"]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} style={{ ...button, background: tab === key ? "#F00028" : "#F1EBDD", color: tab === key ? "white" : "#F00028" }}>{label}</button>)}</nav>{tab === "content" ? <ContentEditor /> : tab === "products" ? <ProductsEditor /> : <SettingsEditor />}</Page>;
}
...
function ContentEditor() {
  ...
  return <Box><div style={{ display: "grid", gridTemplateColumns: "minmax(210px,.7fr) 1.5fr", gap: 24 }}><div><h2 style={heading}>Текст засах</h2><p style={muted}>Нүүр болон дотоод хэсгүүдийн текст, зургийг өөрчилнө.</p><div style={{ display: "grid", gap: 6, marginTop: 16 }}>{Object.entries(sections).map(([key, label]) => <button key={key} onClick={() => setSelected(key)} style={{ textAlign: "left", border: 0, borderRadius: 9, padding: 11, cursor: "pointer", background: selected === key ? "#F1EBDD" : "transparent", color: "#F00028" }}>{label}</button>)}</div></div><div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}><h3 style={heading}>{sections[selected]}</h3><div><button onClick={() => setLang("mn")} style={{ ...button, padding: "7px 11px", background: lang === "mn" ? "#F00028" : "#F1EBDD", color: lang === "mn" ? "white" : "#F00028" }}>MN</button> <button onClick={() => setLang("en")} style={{ ...button, padding: "7px 11px", background: lang === "en" ? "#F00028" : "#F1EBDD", color: lang === "en" ? "white" : "#F00028" }}>EN</button></div></div>{Object.entries(current).map(([key, value]) => <label key={key} style={{ display: "grid", gap: 5, marginBottom: 13, color: "#5C3C2B", fontSize: 13 }}>{key}<textarea value={String(value ?? "")} onChange={(e) => update(key, e.target.value)} rows={key.includes("body") || key.includes("para") || key.includes("quote") ? 4 : 2} style={{ ...input, resize: "vertical" }} /></label>)}<div style={{ borderTop: "1px solid #F1EBDD", paddingTop: 14, marginTop: 10, marginBottom: 16 }}><label style={{ display: "grid", gap: 7, color: "#5C3C2B", fontSize: 13 }}>Энэ хэсгийн зураг солих<input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) uploadImage(file); }} /></label>{image && <img src={mediaUrl(image)} alt="Сонгосон зураг" style={{ width: "100%", maxHeight: 190, objectFit: "cover", borderRadius: 12, marginTop: 12 }} />}</div><button onClick={save} style={{ ...button, background: "#C98A3D", color: "white" }}>Хадгалах</button><span style={{ marginLeft: 12, color: "#F00028" }}>{message}</span></div></div></Box>;
}
...
function ProductsEditor() {
  ...
  return <Box><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}><div><h2 style={heading}>Бүтээгдэхүүн</h2><p style={muted}>Нэр, тайлбар, ангилал, зураг болон нийтэд харагдах байдлыг удирдана.</p></div><button onClick={newProduct} style={{ ...button, background: "#F00028", color: "white" }}>+ Нэмэх</button></div>{editing ? <ProductForm value={editing} setValue={setEditing} save={save} upload={upload} cancel={() => setEditing(null)} message={message} /> : <div style={{ display: "grid", gap: 8 }}>{products.map((p) => <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, borderBottom: "1px solid #F1EBDD", padding: "12px 0" }}><div><b>{p.name_mn || p.name_en || "Нэргүй"}</b><div style={muted}>{p.category || "Ангилалгүй"} · {p.published === false ? "Нууц" : "Нийтэд"}</div></div><div><button onClick={() => setEditing(p)} style={{ ...button, padding: "8px 12px", background: "#F1EBDD", color: "#F00028" }}>Засах</button> <button onClick={() => remove(p.id)} style={{ ...button, padding: "8px 12px", background: "#F1EBDD", color: "#F00028" }}>Устгах</button></div></div>)}</div>}</Box>;
}

function ProductForm({ value, setValue, save, upload, cancel, message }: any) { const fields = ["name_mn", "name_en", "slug", "category", "description_mn", "description_en", "image_url"]; return <div style={{ borderTop: "1px solid #F1EBDD", paddingTop: 18, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 13 }}>{fields.map((key) => <label key={key} style={{ display: "grid", gap: 5, color: "#5C3C2B", fontSize: 13, gridColumn: key.includes("description") || key === "image_url" ? "1 / -1" : undefined }}>{key}<input value={value[key] ?? ""} onChange={(e) => setValue({ ...value, [key]: e.target.value })} style={input} /></label>)}<label><input type="checkbox" checked={value.featured !== false} onChange={(e) => setValue({ ...value, featured: e.target.checked })} /> Нүүрэнд онцлох</label><label><input type="checkbox" checked={value.published !== false} onChange={(e) => setValue({ ...value, published: e.target.checked })} /> Нийтэд харагдах</label><label style={{ gridColumn: "1 / -1" }}>Зураг upload хийх<input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) upload(file); }} /></label><div style={{ gridColumn: "1 / -1" }}><button onClick={save} style={{ ...button, background: "#C98A3D", color: "white" }}>Хадгалах</button><button onClick={cancel} style={{ ...button, background: "#F1EBDD", color: "#F00028", marginLeft: 8 }}>Буцах</button><span style={{ marginLeft: 12, color: "#F00028" }}>{message}</span></div></div>; }

function SettingsEditor() { const [data, setData] = useState<any>({}); const [message, setMessage] = useState(""); useEffect(() => { request("/api/admin/settings").then((r) => r.json()).then(setData); }, []); async function save() { const r = await request("/api/admin/settings", { method: "PATCH", body: JSON.stringify(data) }); setMessage(r.ok ? "Хадгалагдлаа." : "Алдаа гарлаа."); } return <Box><h2 style={heading}>Ерөнхий тохиргоо</h2><p style={muted}>Footer болон холбоо барих хэсгийн мэдээлэл.</p><div style={{ display: "grid", gap: 13, maxWidth: 680, marginTop: 18 }}>{["company_name", "phone", "email", "address_mn", "address_en"].map((key) => <label key={key} style={{ display: "grid", gap: 5 }}>{key}<input value={data[key] ?? ""} onChange={(e) => setData({ ...data, [key]: e.target.value })} style={input} /></label>)}</div><button onClick={save} style={{ ...button, background: "#C98A3D", color: "white", marginTop: 18 }}>Хадгалах</button><span style={{ marginLeft: 12, color: "#F00028" }}>{message}</span></Box>; }
